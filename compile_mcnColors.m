function compile_mcnColors(varargin)
% COMPILE_MCNCOLORS compile the C++/CUDA components of the mcnColors module
...
% -----------------------------------------------------------------------------
  ...
  % Add module files
  lib_src{end+1} = fullfile(root,'src','bits',['rgb2hsv.' ext]) ;
  mex_src{end+1} = fullfile(root,'src',['vl_rgb2hsv.' ext]) ;
  % CPU-specific files
  lib_src{end+1} = fullfile(root,'src','bits','impl','rgb2hsv_cpu.cpp') ;
  % GPU-specific files
  if opts.enableGpu
    lib_src{end+1} = fullfile(root,'src','bits','impl','rgb2hsv_gpu.cu') ;
  end
